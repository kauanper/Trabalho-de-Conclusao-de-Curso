...
Embora essa estrutura contribua para a eficiência operacional, ela também implica em uma diferente distribuição de funções e responsabilidades entre os membros \cite{}. Consequentemente, determinadas informações passam a circular com maior frequência entre aqueles que exercem funções de gestão, o que pode resultar em diferentes níveis de acesso ao conhecimento sobre as atividades do empreendimento, configurando a assimetria informacional \cite{bertolin2008}.

%referenciar a seção de sistema seguros - REFEITO
Esse cenário pode comprometer um dos principais pilares da Economia Solidária: a confiança nas relações internas e nos processos de gestão coletiva \cite{}. Além disso, as ferramentas utilizadas na gestão do empreendimento também desempenham um papel importante na construção dessa confiança, uma vez que são responsáveis por registrar, organizar e disponibilizar informações aos seus membros \cite{}. Para isso, tais ferramentas devem favorecer a transparência, a integridade dos dados e o acesso adequado às informações. Nesse contexto, a relação entre confiança organizacional e confiabilidade dos sistemas de informação será discutida com maior profundidade na Seção X \cite{}.


Esse processo de delegação de responsabilidades, no entanto, faz com que nem todos os membros detenham o mesmo nível de acesso às informações sobre o que ocorre dentro do empreendimento, configurando uma assimetria informacional \cite{bertolin2008}.
