...
\begin{itemize}
    ...
    \item {Especificar os requisitos levantados na hierarquia de Épicos, Features e Histórias de Usuário}, definindo os critérios de aceitação no formato Given-When-Then, de forma a garantir rastreabilidade entre os requisitos e os testes a serem desenvolvidos;
    \item {Modelar a arquitetura do sistema} por meio do Diagrama Entidade-Relacionamento (DER) e do Diagrama de Classes, orientando, respectivamente, a estrutura do banco de dados relacional e a organização do código-fonte da API;
    \item {Implementar a aplicação} seguindo o ciclo \textit{Red-Green-Refactor} do Test-Driven Development (TDD), compondo uma API REST em Go e um cliente web em Next.js/TypeScript, com suíte de testes unitários, de integração, de aceitação e de regressão, de modo a assegurar a confiabilidade das regras de negócio ao longo da evolução do sistema;
    \item {Avaliar a usabilidade da aplicação} junto a associados que não participaram das sessões de prototipação, por meio de Testes de Usabilidade com Usuários sobre as funcionalidades destinadas ao perfil de associado comum, de modo a verificar se a ferramenta favorece o acesso transparente às informações por todo o grupo.
\end{itemize}